...
function preload(){
  ...
  winbg = loadImage("assets/winning background image.jpeg")
  losebg = loadImage("assets/gameover.jpeg")
...
function draw() {
  ...
  if((keyDown("UP_ARROW")||touches.length>0)&&player.y>height/2){
    player.y = player.y-30
  }
  if((keyDown("DOWN_ARROW")||touches.length>0)&&player.y<height){
   player.y = player.y+30
  }
  ...
  if(gameState == "lost"){
    imageMode (CENTER)
    image (losebg,width/2,height/2, width,height)
  ...
  //destroy zombie and player and display a message in gameState "won"
    ...
    imageMode (CENTER)
    image (winbg,width/2,height/2, width,height)
